import "../styling/GettingStarted1.css";

import { useState, useMemo } from "react";
import Select from "react-select";
import countryList from "react-select-country-list";

export default function () {
  const [clientInformation, setClientInformation] = useState({
    firstName: "",
    lastName: "",
    sendNotifs: "",
    companyName: "",
    country: "",
    companyAddress: "",
    state: "",
    city: "",
    zip: "",
  });

  const countryOptions = useMemo(() => countryList().getData(), []);

  const handleSubmit = (event) => {
    event.preventDefault();

    console.log(clientInformation);
  };

  return (
    <>
      <meta charSet="utf-8" />
      {/*<meta name=description content="This site was generated with Anima. www.animaapp.com"/>*/}
      {/* <link rel="shortcut icon" type=image/png href="https://animaproject.s3.amazonaws.com/home/favicon.png" /> */}
      <meta name="viewport" content="width=1920, maximum-scale=1.0" />
      <link
        rel="shortcut icon"
        type="image/png"
        href="https://animaproject.s3.amazonaws.com/home/favicon.png"
      />
      <link
        rel="stylesheet"
        type="text/css"
        href="apply-getting-started-1.css"
      />
      <input
        type="hidden"
        id="anPageName"
        name="page"
        defaultValue="apply-getting-started-1"
      />
      <div className="container-center-horizontal-gettingStarted1">
        <div className="apply-getting-started-1 screen-gettingStarted1">
          <div className="group-439">
            <div className="overlap-group1">
              <div className="group-184">
                <div className="overlap-group">
                  <div className="witt-gen-portal oxygen-bold-blue-dianne-28px">
                    <span className="oxygen-bold-blue-dianne-28px">
                      WittGen
                    </span>
                    <span className="oxygen-light-blue-dianne-28px">
                      Portal
                    </span>
                  </div>
                </div>
                <img className="line-79" src="line-79-1@1x.svg" alt="Line 79" />
              </div>
              <div className="component-13">
                <img
                  className="icon"
                  src="home-fill0-wght400-grad0-opsz48-1-1@2x.svg"
                  alt="icon-home"
                />
                <div className="dashboard inter-normal-blue-dianne-16px">
                  Dashboard
                </div>
              </div>
              <div className="component-14">
                <img
                  className="draft_fill1_wght400_grad0_opsz48-1-1"
                  src="draft-fill1-wght400-grad0-opsz48--1--1-1@2x.svg"
                  alt="draft_FILL1_wght400_GRAD0_opsz48 (1) 1"
                />
                <div className="my-files my inter-semi-bold-blue-dianne-16px">
                  My files
                </div>
              </div>
              <div className="component-22">
                <img
                  className="logout_fill0_wght400_grad0_opsz48-1"
                  src="logout-fill0-wght400-grad0-opsz48-1-1@2x.svg"
                  alt="logout_FILL0_wght400_GRAD0_opsz48 1"
                />
                <div className="logout inter-normal-blue-dianne-16px">
                  Logout
                </div>
              </div>
              <div className="component-13-1">
                <img
                  className="import_contacts_fill"
                  src="import-contacts-fill0-wght400-grad0-opsz48-1-1@2x.svg"
                  alt="import_contacts_FILL0_wght400_GRAD0_opsz48 1"
                />
                <div className="tutorial inter-normal-blue-dianne-16px">
                  Tutorial
                </div>
              </div>
              <div className="component-21">
                <img
                  className="paid_fill0_wght400_grad0_opsz48-1"
                  src="paid-fill0-wght400-grad0-opsz48-1-1@2x.svg"
                  alt="paid_FILL0_wght400_GRAD0_opsz48 1"
                />
                <div className="cost-usage inter-normal-blue-dianne-16px">
                  Cost &amp; Usage
                </div>
              </div>
              <div className="component-23">
                <img
                  className="icon"
                  src="person-fill0-wght400-grad0-opsz48-1-1@2x.svg"
                  alt="icon-user"
                />
                <div className="my-profile my inter-normal-blue-dianne-16px">
                  My Profile
                </div>
              </div>
              <div className="component-18">
                <img
                  className="contact_support_fill"
                  src="contact-support-fill0-wght400-grad0-opsz48--1--1-1@2x.svg"
                  alt="contact_support_FILL0_wght400_GRAD0_opsz48 (1) 1"
                />
                <div className="faq-support inter-normal-blue-dianne-16px">
                  FAQ / Support
                </div>
              </div>
              <div className="component-19">
                <img
                  className="icon"
                  src="call-fill0-wght400-grad0-opsz48--1--1-1@2x.svg"
                  alt="icon-call"
                />
                <div className="contact-us inter-normal-blue-dianne-16px">
                  Contact Us
                </div>
              </div>
              <div className="component-24">
                <img
                  className="icon"
                  src="settings-fill0-wght400-grad0-opsz48-1-1@2x.svg"
                  alt="icon-settings"
                />
                <div className="settings inter-normal-blue-dianne-16px">
                  Settings
                </div>
              </div>
            </div>
          </div>
          <div className="frame-447">
            <div className="frame-350">
              <h1 className="title inter-semi-bold-slate-gray-32px">
                GH-13728930
              </h1>
              <div className="frame-258">
                <div className="frame-49">
                  <div className="getting-started inter-semi-bold-blue-dianne-16px">
                    Getting started
                  </div>
                  <div className="rectangle-228-1" />
                </div>
                <div className="frame-49">
                  <div className="metadatabase-input inter-normal-oslo-gray-16px">
                    Metadatabase Input
                  </div>
                  <div className="rectangle-228" />
                </div>
                <div className="frame-49">
                  <div className="database-input inter-normal-oslo-gray-16px">
                    Database Input
                  </div>
                  <div className="rectangle-228" />
                </div>
                <div className="frame-49">
                  <div className="pay-submit inter-normal-oslo-gray-16px">
                    Pay &amp; Submit
                  </div>
                  <div className="rectangle-228" />
                </div>
              </div>
              <div className="frame-213">
                <div className="component-33">
                  <div className="researchers inter-semi-bold-blue-dianne-14px">
                    Discard &amp; Exit
                  </div>
                </div>
                <div className="component-31">
                  <div className="researchers inter-semi-bold-white-14px">
                    Save &amp; Exit
                  </div>
                </div>
              </div>
            </div>
          </div>
          <form onSubmit={handleSubmit}>
            <div>
              <div className="group-438">
                <div className="frame-616">
                  <div className="component-33-1">
                    <button
                      type="submit"
                      className="researchers-1 researchers-3 inter-semi-bold-white-14px"
                    >
                      Next
                    </button>
                  </div>
                </div>
              </div>
              <div className="frame-447-1">
                <div className="frame-40 inter-normal-slate-gray-14px">
                  <div className="frame">
                    <div className="application">Application type</div>
                    <div className="frame-290-1">
                      <div className="researchers-2 researchers-3">
                        Researchers
                      </div>
                    </div>
                  </div>
                  <div className="frame">
                    <div className="application">Application ID</div>
                    <div className="frame-290-2">
                      <div className="gh-13728930">GH-13728930</div>
                    </div>
                  </div>
                </div>
                <div className="component-85">
                  <div className="group-405">
                    <div className="frame-496">
                      <div className="client-information inter-semi-bold-blue-dianne-16px">
                        Client Information
                      </div>
                      <div className="group-401">
                        <div className="frame-40-1 frame-40-3">
                          <div className="rectangle-253" />
                          <div className="same-as-profile inter-normal-tundora-14px">
                            Same as profile
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="frame-449">
                      <div className="frame-40-2 frame-40-3">
                        <div className="frame-304">
                          <div className="frame inter-normal-slate-gray-14px">
                            <div className="first-name">First name</div>
                            <div className="frame-290">
                              <div className="group-234">
                                <input
                                  value={clientInformation.firstName}
                                  onChange={(event) => {
                                    setClientInformation({
                                      ...clientInformation,
                                      firstName: event.target.value,
                                    });
                                  }}
                                  className="x1234emailcom"
                                />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className="frame-40-2 frame-40-3">
                        <div className="frame-304">
                          <div className="frame inter-normal-slate-gray-14px">
                            <div className="last-name">Last name</div>
                            <div className="frame-290">
                              <div className="group-234">
                                <input
                                  className="x1234emailcom-7"
                                  value={clientInformation.lastName}
                                  onChange={(event) => {
                                    setClientInformation({
                                      ...clientInformation,
                                      lastName: event.target.value,
                                    });
                                  }}
                                />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="frame-304-1">
                    <div className="group-317">
                      <div className="group-298">
                        <div className="frame-40-1 frame-40-3">
                          <div className="send-notifications-to inter-normal-slate-gray-14px">
                            Send notifications to
                          </div>
                          <p className="for-receiving-updates-to-your-file-only inter-semi-bold-tahiti-gold-11px">
                            For receiving updates to your file ONLY
                          </p>
                        </div>
                        <div className="frame-290-3">
                          <div className="group-234-2">
                            <input
                              value={clientInformation.sendNotifs}
                              onChange={(event) => {
                                setClientInformation({
                                  ...clientInformation,
                                  sendNotifs: event.target.value,
                                });
                              }}
                              className="exampleemailcom inter-normal-silver-14px"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="frame-40">
                    <div className="frame-304">
                      <div className="frame inter-normal-slate-gray-14px">
                        <div className="company-name company">Company name</div>
                        <div className="frame-290">
                          <div className="group-234">
                            <input
                              value={clientInformation.companyName}
                              onChange={(event) => {
                                setClientInformation({
                                  ...clientInformation,
                                  companyName: event.target.value,
                                });
                              }}
                              className="x1234emailcom-7"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="frame inter-normal-slate-gray-14px">
                      <div className="country">Country</div>
                      <div className="frame-290-4">
                        <div className="group-234-3">
                          {/* <div className="select-a-country">
                            Select a country
                          </div> */}

                          <Select
                            options={countryOptions}
                            value={clientInformation.country}
                            onChange={(event) => {
                              setClientInformation({
                                ...clientInformation,
                                country: event,
                              });
                            }}
                          />
                        </div>
                        {/* <img
                          className="frame-607"
                          src="frame-607@2x.svg"
                          alt="Frame 607"
                        /> */}
                      </div>
                    </div>
                  </div>
                  <div className="frame-40">
                    <div className="group">
                      <div className="frame-304">
                        <div className="group">
                          <div className="group-298-1 inter-normal-slate-gray-14px">
                            <div className="company-address company">
                              Company address
                            </div>
                            <div className="frame-290-5">
                              <div className="group-234">
                                {/* <div className="x1234emailcom-7">
                                  1234@email.com
                                </div> */}
                                <input
                                  value={clientInformation.companyAddress}
                                  onChange={(event) => {
                                    setClientInformation({
                                      ...clientInformation,
                                      companyAddress: event.target.value,
                                    });
                                  }}
                                  className="x1234emailcom-7"
                                />
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="frame-40">
                    <div className="frame">
                      <div className="state-province inter-normal-slate-gray-14px">
                        State/ province
                      </div>
                      <div className="frame-290">
                        <div className="group-234-1">
                          {/* <div className="x1234emailcom-7 inter-bold-slate-gray-14px">
                            1234@email.com
                          </div> */}
                          <input
                            value={clientInformation.state}
                            onChange={(event) => {
                              setClientInformation({
                                ...clientInformation,
                                state: event.target.value,
                              });
                            }}
                            className="x1234emailcom-7 inter-bold-slate-gray-14px"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="group-298-2">
                      <div className="frame">
                        <div className="city inter-normal-slate-gray-14px">
                          City
                        </div>
                        <div className="frame-290">
                          <div className="group-234-1">
                            {/* <div className="x1234emailcom-7 inter-bold-slate-gray-14px">
                              1234@email.com
                            </div> */}
                            <input
                              value={clientInformation.city}
                              onChange={(event) => {
                                setClientInformation({
                                  ...clientInformation,
                                  city: event.target.value,
                                });
                              }}
                              className="x1234emailcom-7 inter-bold-slate-gray-14px"
                            />
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="group-299">
                    <div className="frame">
                      <div className="zip-postal-code inter-normal-slate-gray-14px">
                        Zip/ Postal code
                      </div>
                      <div className="frame-290-6">
                        <div className="group-234-4">
                          {/* <div className="x1234emailcom-7 inter-bold-slate-gray-14px">
                            1234@email.com
                          </div> */}
                          <input
                            value={clientInformation.zip}
                            onChange={(event) => {
                              setClientInformation({
                                ...clientInformation,
                                zip: event.target.value,
                              });
                            }}
                            className="x1234emailcom-6 inter-bold-slate-gray-14px"
                          />
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </form>
          <div className="frame-502">
            <div className="group-447">
              <div className="frame-564">
                <p className="upgrade-freemium-to-6-months inter-semi-bold-cerulean-20px">
                  Upgrade Freemium to 6 months
                </p>
                <div className="frame-565">
                  <div className="frame-563">
                    <p className="agree-to-wittgens-u inter-normal-blue-dianne-14px">
                      Agree to Wittgen’s use of sample &amp; data information
                      and enjoy 6 months unlimited access to Wittgen’s services
                      for free. Effective immediately.
                    </p>
                    <div className="component-86">
                      <div className="group-234-5">
                        <div className="read-terms-conditions">
                          Read terms &amp; conditions
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="group-448">
              <div className="overlap-group-1">
                <div className="x100-sample inter-semi-bold-black-16px">
                  <span className="span0 inter-semi-bold-black-16px">$100</span>
                  <span className="inter-normal-black-16px">/ sample</span>
                </div>
                <img
                  className="line-109"
                  src="line-109@1x.svg"
                  alt="Line 109"
                />
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}
